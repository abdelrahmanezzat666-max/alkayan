...
import { fromDoc, type RoleRecord, type WithId } from "../../lib/firestore.js";
import { HttpError } from "../../lib/http-error.js";
import { ALL_PERMISSIONS } from "../../lib/permissions.js";

function roleDto(role: WithId<RoleRecord>) {
  return {
    id: role.id,
    name: role.name,
...
async function assertUniqueRoleName(name: string, excludeId?: string) {
  const snapshot = await db.collection("roles").where("name", "==", name).limit(1).get();
  const existing = snapshot.docs[0] ? fromDoc(snapshot.docs[0] as DocumentSnapshot<RoleRecord>) : null;
...
export async function createRole(name: string, permissionNames: string[]) {
  ...
  const role = fromDoc((await ref.get()) as DocumentSnapshot<RoleRecord>)!;
  return roleDto(role);
}

export async function updateRolePermissions(roleId: string, permissionNames: string[]) {
  assertValidPermissions(permissionNames);
  ...
  return roleDto(fromDoc((await ref.get()) as DocumentSnapshot<RoleRecord>)!);
}
