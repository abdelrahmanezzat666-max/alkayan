...
import { fromDoc, toDate, type InvitationRecord, type UserRecord } from "../../lib/firestore.js";
...
export async function login(email: string, password: string) {
  ...
  const user = users.docs[0] ? fromDoc(users.docs[0] as DocumentSnapshot<UserRecord>) : null;
...
export async function acceptInvitation(token: string, password: string) {
  ...
  const invitation = invitations.docs[0] ? fromDoc(invitations.docs[0] as DocumentSnapshot<InvitationRecord>) : null;
  ...
  const user = fromDoc(userDoc as DocumentSnapshot<UserRecord>);
  if (!user) {
