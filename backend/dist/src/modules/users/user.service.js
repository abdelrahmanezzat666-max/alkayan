...
async function roleByName(name) {
    ...
    return result.docs[0] ? fromDoc(result.docs[0]) : null;
}
...
async function resolveAccess(userId, access, useDefaultEmployeeRole = false) {
...
async function assertUniqueEmail(email, excludeId) {
    ...
    const existing = result.docs[0] ? fromDoc(result.docs[0]) : null;
...
export async function createUser(input, actorId) {
    ...
    const roleIds = await resolveAccess(createdRef.id, {
    ...
    const created = fromDoc((await createdRef.get()));
...
export async function updateUser(id, input) {
    ...
    return toUserDto(fromDoc((await userRef(id).get())));
}
...
export async function resendInvitation(id, actorId) {
    const user = fromDoc((await userRef(id).get()));
    if (!user) {
        throw new HttpError(404, "User not found");
    }
    if (user.password) {
